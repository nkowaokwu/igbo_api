...
export const sortDocsBy = (searchWord: string, docs: Word[], key: string, version: Version, regex: SearchRegExp) =>
  docs
    // Filters out documents with headwords that don't resemble the search word
    .filter((doc) => {
      const normalizedSearchWord = removePrefix(searchWord.normalize('NFC'));
      const docValue = get(doc, key);
      if (Math.abs(docValue.length - normalizedSearchWord.length) <= MAX_STRING_LIMIT) {
        return doc;
      }
      return false;
    })
    .sort((prevDoc, nextDoc) => {
      const normalizedSearchWord = removePrefix(searchWord.normalize('NFC'));
